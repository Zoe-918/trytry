...
<body>
    ...
    <script>
        try {
            ...
            function performSearch(value, forcePopup) {
                const term = value.toLowerCase().trim();
                const domTables = document.querySelectorAll('.table');
                let matchedTables = [];

                if (term === "") {
                    domTables.forEach(t => t.classList.remove('highlight', 'dimmed'));
                    return;
                }
                ...
                domTables.forEach(t => {
                    ...
                    if (index.includes(term)) {
                        ...
                        matchedTables.push(t.dataset.id);
                    } else {
                ...
                if (!forcePopup && matchedTables.length === 1) {
                    const targetTable = tablesData.find(t => t.id === matchedTables[0]);
                    openModal(targetTable);
                }
                else if (forcePopup && matchedTables.length > 0) {
                    const targetTable = tablesData.find(t => t.id === matchedTables[0]);
                    openModal(targetTable);
                }
            }
            ...
            alert("程式碼發生錯誤，請截圖給開發者：" + error.message);
            console.error(error);
        }
    </script>
</body>
